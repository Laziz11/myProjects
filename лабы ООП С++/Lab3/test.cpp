...
void testUnitStackWithDefaultConstructor() {
    //Тестирование конструктора по умолчанию
    UnitStack unit;
    assert(Unit::NAME_DEFAULT == unit.getName());
    assert(Unit::PRICE_DEFAULT == unit.getPrice());
    assert(10 == unit.getVolume());
}

void testUnitStackWithParameters() {
    //Тестирование инициализирующего конструктора
    UnitStack unit("Item_1",123);
    assert("Item_1" == unit.getName());
    assert(123 == unit.getPrice());
    assert(6 == unit.getVolume());
}

void testUnitStackVolumes() {
    //Тестирование расчета объема
    UnitStack unit("Item_1",66);
    assert(4 == unit.getVolume());
    unit.setPrice(77) ;
    assert(6 == unit.getVolume());
    unit.setPrice(88) ;
    assert(8 == unit.getVolume());
    unit.setPrice(999) ;
    assert(9+1+4+1+4 == unit.getVolume());
}

void testUnitInfo() {
    //Тестирование метода информации о классе
    Unit unit1("Item_1",123);
    UnitStack unit2("Item_1",123);
    assert("Вещь"==unit1.getTypeName()) ;
    assert("Стопка монет"==unit2.getTypeName()) ;
}
...
void testClasses() {
    ...
    testUnitStackWithDefaultConstructor() ;
    testUnitStackWithParameters() ;
    testUnitStackVolumes() ;
    testUnitInfo() ;
